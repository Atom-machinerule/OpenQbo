...
int Orbit::loadOrbit(string load_path)
{
	...
	loadObjectsNames(load_path);
	printf("Loaded %u names\n", (unsigned int)objects_.size());

	if(objects_.size()>1)
	{
		printf("\nLoading BOW Vocabulary...\n");
		if(loadVocabulary(load_path) == false)
			return -1;
		printf("BOW Vocabulary loaded!\n");


		printf("\nLoading SVM Classifiers...\n");
		if(loadSVMClassifiers() == false)
			return -1;
		printf("SVM Classifiers loaded!\n");
	}
...
//			if(input_image.at<float>(i,j)==0)
//				input_image.at<unsigned char>(i,j) = 0;

    //Detect keypoints from grayscale image
	descriptor_detector_->detect(input_image, extracted_keypoints, mask);
...
int Orbit::trainSVMClassifiers()
{
	...
	for(unsigned int i = 0; i<objects_.size(); i++) //For each object
	{
        ...
        //cout<<"\t - ---- Responses: "<<responses.cols<<", "<<responses.rows<<"  Data: "<<train_data.cols<<", "<<train_data.rows<<endl;
...
//				mask.at<unsigned char>(i,j) = 0;
    ...
    //Detect keypoints from grayscale image
	descriptor_detector_->detect(image, extracted_keypoints, mask);
    //Extract descriptors from grayscale image
